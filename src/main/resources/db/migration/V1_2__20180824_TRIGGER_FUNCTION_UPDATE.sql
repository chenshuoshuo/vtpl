 ...
 begin
		if NEW.lng is not null and NEW.lat is not null and NEW.location_time is not null then
                ...
                if _data_count = 0 then
    				_sql = 'insert into ' || _table_name || ' (userid, realname, gender, account_mac, org_code, org_name, lng, lat, floorid, location_time, in_door, in_school)
                ...
                if _data_count = 0 then
                    _sql = 'insert into location_history_day (userid, realname, gender, account_mac, org_code, org_name, lng, lat, floorid, location_time, in_door, in_school)
						values ('''|| NEW.userid || ''', '''|| NEW.realname || ''', ''' || NEW.gender || ''', ''' || NEW.account_mac || ''', ''' || NEW.org_code || ''', ''' || NEW.org_name
						|| ''', ' || NEW.lng || ', ' || NEW.lat || ', ''' || NEW.floorid || ''', ''' || NEW.location_time || ''', ' || NEW.in_door || ', ' || NEW.in_school || ')';
			    	execute _sql;
                end if;
                ...
                select count(*) from location_history_month where to_char(location_time, 'yyyy-mm-dd hh24') = to_char(NEW.location_time, 'yyyy-mm-dd hh24')
                    and userid = NEW.userid into _data_count;
                if _data_count = 0 then
                    _sql = 'insert into location_history_month (userid, realname, gender, account_mac, org_code, org_name, lng, lat, floorid, location_time, in_door, in_school)
						values ('''|| NEW.userid || ''', '''|| NEW.realname || ''', ''' || NEW.gender || ''', ''' || NEW.account_mac || ''', ''' || NEW.org_code || ''', ''' || NEW.org_name
						|| ''', ' || NEW.lng || ', ' || NEW.lat || ', ''' || NEW.floorid || ''', ''' || NEW.location_time || ''', ' || NEW.in_door || ', ' || NEW.in_school || ')';
			    	execute _sql;
                end if;
                ...
                select count(*) from location_history_year where to_char(location_time, 'yyyy-mm-dd') = to_char(NEW.location_time, 'yyyy-mm-dd')
                    and userid = NEW.userid into _data_count;
                if _data_count = 0 then
                    _sql = 'insert into location_history_year (userid, realname, gender, account_mac, org_code, org_name, lng, lat, floorid, location_time, in_door, in_school)
						values ('''|| NEW.userid || ''', '''|| NEW.realname || ''', ''' || NEW.gender || ''', ''' || NEW.account_mac || ''', ''' || NEW.org_code || ''', ''' || NEW.org_name
						|| ''', ' || NEW.lng || ', ' || NEW.lat || ', ''' || NEW.floorid || ''', ''' || NEW.location_time || ''', ' || NEW.in_door || ', ' || NEW.in_school || ')';
			    	execute _sql;
                end if;
